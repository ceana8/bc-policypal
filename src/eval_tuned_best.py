...
def run_evaluation(top_k=4, oos_threshold=0.82):
    load_dotenv()

    storage_context = StorageContext.from_defaults(persist_dir="storage_tuned_400")
    index = load_index_from_storage(storage_context)
    query_engine = index.as_query_engine(similarity_top_k=top_k)

    print(f"\n{'=' * 60}")
    print(f"  BCPolicyPal Best-Case Tuned Evaluation (top_k={top_k}, oos_threshold={oos_threshold})")
    print(f"{'=' * 60}\n")
...
if __name__ == "__main__":
    run_evaluation(top_k=4, oos_threshold=0.82)
