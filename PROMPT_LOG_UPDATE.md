# Prompt Log Update - Lab 7.3 Retrieval Tuning

Date: 2026-05-25
Project: bc-policypal (github colplot)
Scope: Complete prompt log plus threshold calibration update

## Full Prompt Log (Start to End)
...
## 15. Threshold Calibration Update

This update captures threshold calibration details and clarifies why threshold tuning was run on a fixed best retrieval setup.

What was updated:
- Clarified that OOS accuracy = 0.000 was the initial heuristic result from threshold 0.28.
- Added threshold calibration details from a dedicated sweep.
- Documented why chunk size and top_k were fixed during threshold calibration.

Threshold calibration prompt:
- Calibrate OOS threshold after selecting the best retrieval setup.

Fixed setup for threshold testing:
- chunk_size = 400
- top_k = 4

Why this setup was fixed:
- Threshold tuning should isolate one variable at a time.
- Keeping chunk and top_k fixed ensures changes come from threshold only.
- This avoids mixing retrieval tuning effects with OOS cutoff tuning effects.
...
Best-case metrics at threshold 0.82 (chunk 400, top_k 4):
